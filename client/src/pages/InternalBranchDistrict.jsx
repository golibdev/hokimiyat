            ...
            <div className="row g-5">
               {internalBranches.map(item => (
                  <div className="col-lg-12" key={item._id}>
                     <Link style={{ fontSize: '50px'}} to={`/district/${districtId}/branches/${branchId}/internal-branches/${item._id}`} id="btn-hover" className="btn-hover btn btn-lg text-uppercase fw-bold me-5 w-100">
                        <span>{item.name}</span>
                     </Link>
                  </div>
               ))}
...
export default InternalBrancheDistrict